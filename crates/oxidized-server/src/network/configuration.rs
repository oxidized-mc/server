...
/// 1. Send server brand (`CustomPayload` — `minecraft:brand`)
/// 2. Send `ClientboundUpdateEnabledFeaturesPacket` (vanilla features)
/// 3. Send `ClientboundSelectKnownPacksPacket`
/// 4. Receive `ServerboundSelectKnownPacksPacket`
/// 5. Send `ClientboundRegistryDataPacket` × N (one per synchronized registry)
/// 6. Send `ClientboundUpdateTagsPacket` (all tag registries with entries)
/// 7. Send `ClientboundFinishConfigurationPacket`
/// 8. Receive `ServerboundFinishConfigurationPacket`
/// 9. Transition to PLAY state
///
...
pub async fn handle_configuration(
    ...
    // 2. Send enabled features (vanilla feature set) — must precede registries.
    ...
    // 3. Send SelectKnownPacks — we claim to have the vanilla core pack
    ...
    // 4. Receive serverbound packets until we get SelectKnownPacks.
    ...
    // 5. Send all synchronized registries (full data, ignoring known-pack
    ...
    // 6. Send tags (block, item, fluid, entity_type, enchantment, etc.)
